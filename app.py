...
# 스마트폰에서 가장 완벽하게 열리는 성경 코드 매핑 테이블
bible_url_mapping = {
    ...
    <script>
        ...
        function toggleCheck(element) {
            ...
            if (element.classList.contains('checked')) {
                ...
                let cleanBook = rawBook.split('(')[0].trim();
                cleanBook = cleanBook.split('/')[0].trim();
                
                const bibleCode = mapping[rawBook] || "gen";
                
                document.getElementById('bibleInfo').innerText = `📍 ${cleanBook} ${chapter}장`;
                
                // 💡 스마트폰 브라우저에서 오류 없이 다이렉트로 완벽 작동하는 성경 본문 주소로 수리!
                document.getElementById('bibleLink').href = `https://m.mabible.com/search_word.php?version=woo&work=read&book=${bibleCode}&chap=${chapter}`;
